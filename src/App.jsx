...
function Hero() {
  ...
  return <section className="hero">
    ...
    <img className="hero-fox" src={asset('fox-emblem.png')} alt="Raposa e paisagem da Via Fox Tour" />
...
function Destinations() {
  return <><section className="intro" id="destinos"><div><p className="eyebrow">ESCOLHA SEU CENÁRIO</p><h2>Destinos que ficam<br />na memória.</h2></div><div className="intro-detail"><img src={asset('fox-emblem.png')} alt="" aria-hidden="true" /><p>Do litoral de águas transparentes a paisagens que surpreendem, cada passeio é organizado com cuidado, segurança e leveza.</p></div></section><section className="destination-grid">{destinations.map((destination) => <article className={`destination ${destination.className || ''}`} key={destination.name}><img src={destination.image} alt={`Paisagem de ${destination.name}`} /><div className="card-overlay"><span className="tag">{destination.state}</span><h3>{destination.name}</h3><button onClick={() => askViaWhatsApp(destination.name)}>Conhecer <b>↗</b></button></div></article>)}</section></>
}

function Experience() {
  return <section className="experience" id="experiencia"><div className="experience-photo"><img src={asset('maceio.jpg')} alt="Maceió e seu mar azul" /><span className="stamp"><img src={asset('fox-emblem.png')} alt="" aria-hidden="true" /></span></div><div className="experience-copy"><p className="eyebrow">UMA VIAGEM TRANQUILA</p><h2>Você cuida das lembranças. Nós cuidamos do caminho.</h2><p>Transporte seguro, horários organizados e atendimento próximo para transformar o planejamento em uma parte boa da viagem.</p><div className="reasons"><div><i>01</i><h3>Seguro e pontual</h3><p>Você embarca com tranquilidade e chega no tempo certo.</p></div><div><i>02</i><h3>Do seu jeito</h3><p>Atendimento personalizado para cada roteiro e grupo.</p></div></div><WhatsAppButton className="button blue">Falar com a Via Fox</WhatsAppButton></div></section>
}

function Contact() {
  return <section className="contact" id="contato"><img className="contact-fox" src={asset('fox-emblem.png')} alt="" aria-hidden="true" /><div className="contact-content"><p className="eyebrow light">VAMOS COMEÇAR?</p><h2>Seu destino está<br />mais perto do que parece.</h2><WhatsAppButton>Chamar no WhatsApp</WhatsAppButton><a className="instagram" href="https://www.instagram.com/viafoxtour/" target="_blank" rel="noreferrer">Siga <strong>@viafoxtour</strong> no Instagram ↗</a></div></section>
}
